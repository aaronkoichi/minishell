...
// 	ast_tree = parse_token(*token_list, vars);
// 	if (ast_tree == NULL)
// 	{
// 		free(input);
...
void	catch_sigint_exit_code(t_vars *vars)
{
	if (g_signal == 130)
	{
		vars->exit_code = 130;
...
void	start_usr_input(t_vars *vars)
{
	...
	while (1)
	{
		...
		if (input == NULL)
			break ;
		catch_sigint_exit_code(vars);
		add_history(input);
		ast_tree = parse_input(&token_list, input, vars);
		set_signal_noninteract();
		if (ast_tree == NULL)
			continue ;
		exec_main(vars->ast_tree, vars, ast_tree, token_list);
